...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        if ($user) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM customercare WHERE FirstName = :FirstName AND MiddleName = :MiddleName AND LastName = :LastName AND Email = :Email AND feedback = :feedback");
            $stmt->execute([
                ':FirstName' => $FirstName,
                ':MiddleName' => $MiddleName,
                ':LastName' => $LastName,
...
<h1 style="width: 100%; background-color: #21395f; color: white; text-align: center; padding: 6px;">Feedback</h1>
...
<form method="POST" action="feedback2.php">
  ...
  <div class="mb-4">
    <label class="block text-gray-700 text-sm font-bold mb-2" for="Query">Your Feedback</label>
    <textarea name="feedback" id="feedback" rows="4" placeholder="Describe your issue" required class="shadow appearance-none border rounded w-full py-2 px-3"></textarea>
  </div>
